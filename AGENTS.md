...
Done = evidence. Report exact commands/results, runtime checks, files, contract/migration/ADR changes, assumptions, unresolved conflicts and handoff using docs/ai/HANDOFF-TEMPLATE.md. Your assignment boundary is the ordered task queue explicitly approved for the current batch. Continue automatically only through dependency-ready tasks belonging to the same acting human AND workstream: verify owner and allowed paths/leases before each task, then implement/test/inspect/commit/checkpoint. When the next task belongs to another owner, STOP IMPLEMENTATION, publish the tested provisional integration SHA and hand off to that owner. WS3 may coordinate/import/test or continue authorized WS3 work; it may not take WS1/WS2 implementation or review fixes. Owner unavailable is not reassignment. Only an explicit senior instruction recorded in CURRENT-WORK can reassign. See ADR-014 and the ownership-preserving section of docs/plans/LONG-RUNNING-WORK.md. No new prompt or PR per ordinary subtask. Stop at batch completion or defined escalation/session limits. Before final handoff complete the bounded two-pass freshness protocol in docs/plans/LONG-RUNNING-WORK.md. After Pass 2 stop; record the exact cutoff, final head, evidence and remaining risks. Interrupted verification is UNVERIFIED, never a freshness pass. Production promotion and destructive live actions require the designated human release approval.
