...
GENERATED_WALK_PARTS={'.git','node_modules','.next','playwright-report','test-results','coverage','dist','out'}

def generated_path(path):
    return any(part in GENERATED_WALK_PARTS for part in Path(path).parts)

def skip_markdown_link_check(path):
    return generated_path(path) or 'artifact' in Path(path).parts

def skip_secret_tripwire(path):
...
def main():
    ...
    for p in ROOT.rglob('*.md'):
        if skip_markdown_link_check(p):continue
    ...
    for p in ROOT.rglob('*'):
        if not p.is_file() or skip_secret_tripwire(p) or p.suffix in ['.png','.zip','.pyc']:continue
