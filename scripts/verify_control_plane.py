...
for p in work:
    ...
    plan=(p/'IMPLEMENTATION-PLAN.md').read_text()
...
require(actual==manifest,'Approved reference bytes changed')
require((ROOT/'docs/contracts/domain.generated.ts').read_text()==generate(),'Generated TS differs; regenerate from schema')
...
for file in (ROOT/'docs/contracts').glob('*.json'):pointers(json.loads(file.read_text()),file)
