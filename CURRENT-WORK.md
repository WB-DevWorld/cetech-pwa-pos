...
| CP-05 | WS3 | ws3/cp-05-pin-toolchain-and-create-next.js-ci-scaffold | IMPLEMENTED / local evidence recorded; human review required | CP-03 (met on main 15287691); toolchain verified from official registries 2026-09-12 | v1.0.0 unchanged; canonical re-export wiring deferred | Shared foundation first |
...
- **Editor:** WS3 senior / @wbdevworld (this CP-05 implementation agent).
- **Task:** CP-05 — Pin toolchain and create Next.js/CI scaffold (GitHub issue #5).
- **Branch / worktree:** `ws3/cp-05-pin-toolchain-and-create-next.js-ci-scaffold` at `C:\Users\Jane\Desktop\Learning 2026\Cursor\cetech-pwa-pos-ws3-cp-05`.
...
- **Unrelated dirty checkout preserved:** the original `main` worktree still has uncommitted `reference/frontend-approved/artifact/**` edits. Those files are not touched here.
...
- **Narrow extra paths recorded before edit (assignment §7):**
  ...
  - `scripts/verify_control_plane.py` — **required after first `pnpm install`**. The foundation verifier currently walks every `*.md` and every file under the repo. Dependency trees (`node_modules`) contain third-party README links that are not project contracts. Without skipping generated/install trees, `python scripts/verify_control_plane.py` fails as soon as the CP-05 lockfile is installed. Narrow change: skip `node_modules`, `.next`, `playwright-report`, `test-results`, `coverage`, `dist`, and `out` in the markdown-link and secret-tripwire walks. No contract/reference/hash behavior change.
