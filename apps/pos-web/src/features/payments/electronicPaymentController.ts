...
export function createElectronicPaymentController(ports: ElectronicPaymentPorts) {
  ...
  async function initializeUnlocked(transactionId: string, tender: ElectronicTender): Promise<void> {
    ...
    setSession({
      ...
      transactionId,
      tender,
      message: "Payment is initializing. Do not charge again.",
      warning: "Do not charge again.",
      doNotChargeAgain: true,
      presentAllowed: false,
      resolveAllowed: true,
      ...
      try {
        ...
        if (probe.kind === "unknown" || (probe.kind === "result" && !probe.value.ok && shouldResolveFailure(probe.value))) {
          setSession({
            ...
            paymentId: session.paymentId,
            tender: input.tender,
            message: "Payment status is uncertain. Do not charge again. Resolve the existing payment.",
            warning: "Do not charge again.",
            doNotChargeAgain: true,
            presentAllowed: false,
            resolveAllowed: true,
    ...
    async noteBrowserCallback(): Promise<void> {
      ...
      try {
        setSession({
          ...
          message: "A browser or provider callback is not payment truth. Do not treat this as verified. Checking the existing payment.",
          warning: "Do not charge again.",
