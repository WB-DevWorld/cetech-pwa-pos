...
import { executeReturn } from "./execute";
import { isReturnExecuteRequest } from "./schema";

export async function handleExecuteReturn(input: {
  ...
  if (!isReturnExecuteRequest(input.body)) {
    const body = authFailure("VALIDATION_ERROR", "ReturnExecuteRequest is invalid", guard.correlationId);
  ...
  const stored = await input.returnStore.getReturn(input.body.returnId);
  if (!stored) {
