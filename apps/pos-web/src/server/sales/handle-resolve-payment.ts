...
import { isPaymentLookup } from "./schema";
...
export async function handleResolvePayment(input: HandleResolvePaymentInput): Promise<HandleResolvePaymentResponse> {
  ...
  if (!payment || payment.transactionId !== input.body.transactionId) {
    const body = apiFailure("NOT_FOUND", "payment evidence was not found", guard.correlationId);
    return { status: httpStatusFor(body.error.code), body, headers: guard.headers };
  }
  if (payment.saleId !== sale.prepared.saleId) {
    const body = apiFailure("REQUIRES_ATTENTION", "payment evidence is bound to a different sale", guard.correlationId);
    return { status: httpStatusFor(body.error.code), body, headers: guard.headers };
  }
  return {
    status: 200,
    body: { ok: true, data: toPaymentState(payment), correlationId: guard.correlationId },
