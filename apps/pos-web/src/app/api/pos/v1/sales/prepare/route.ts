...
import { handlePrepareSale } from "../../../../../../server/sales/handle-prepare-sale";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const result = await handlePrepareSale({
    ...
    quoteSnapshots: composed.runtime.quoteSnapshots,
    prepareIntents: composed.runtime.prepareIntents,
...
function composeHandlers(request: NextRequest) {
  try {
    ...
    const sessionStore = composeStaffSessionStore(process.env, fetchImpl);
    const runtime = composeCheckoutRuntime(process.env, fetchImpl);
    ...
    const body = authFailure(
      ...
      "durable staff session, checkout store, and Woo bridge are required",
