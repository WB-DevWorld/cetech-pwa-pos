...
export function PassiveTabNotice({ passive }: { readonly passive: boolean }) {
  ...
  return (
    <div className="banner warning operational-banner" role="alert" data-passive-tab="true">
      <strong>This POS window is in passive mode.</strong>
      <span>Use the active POS window for changes. This window stays read-only so two tabs do not compete over local transaction state.</span>
    </div>
  );
}
  ...
  return (
    <section className="operational-surface" aria-labelledby="store-health-title">
      <div className="page-head">
        ...
        {onRebuildCatalog ? <button className="btn" type="button" onClick={onRebuildCatalog}>Rebuild catalog</button> : null}
      </div>
      ...
      {state === "error" ? (
        <div className="banner danger operational-banner" role="alert">
          <strong>Store Health could not be refreshed.</strong>
          <span>{errorMessage ?? "Last known local state can remain visible, but current provider health is unverified."}</span>
      ...
      <div className="operational-metrics" aria-label="Store health summary">
        ...
        <div className="card operational-metric"><span className="eyebrow">Needs attention</span><strong>{attentionCount}</strong></div>
        <div className="card operational-metric"><span className="eyebrow">Build</span><strong className="operational-build">{health?.buildId ?? "Unverified"}</strong></div>
      </div>

      {state === "loading" ? (
        <div className="card card-pad operational-state" role="status" aria-live="polite"><div className="operational-spinner" aria-hidden="true" /><strong>Checking store health…</strong></div>
      ) : null}

      {state !== "loading" && checks.length === 0 ? (
        <div className="card card-pad operational-state" role="status">
          <div><strong>No current health checks are available.</strong><p>Do not assume services are healthy until the HealthPort supplies current checks.</p></div>
        </div>
      ) : null}
        ...
        <div className="operational-health-list">
          {checks.map((check) => (
            <div className="operational-health-row" key={check.id}>
              <div><strong>{check.id}</strong><span>{check.message}</span><small>Checked {new Date(check.checkedAt).toLocaleString()}</small></div>
      ...
      <section className="card card-pad operational-version" aria-labelledby="version-recovery-title">
        ...
        <div className="operational-version-grid">
          <div><span>API contract</span><strong>{health?.contractVersion ?? "Unverified"}</strong></div>
          <div><span>Application</span><strong>{appVersion ?? health?.buildId ?? "Unverified"}</strong></div>
          <div><span>Local schema</span><strong>{localSchemaVersion ?? "Unverified"}</strong></div>
          <div><span>Device</span><strong>{deviceName ?? "Unassigned"}</strong></div>
        </div>
        <div className="operational-actions">
          {onFixApp ? <button className="btn" type="button" onClick={onFixApp}>Fix App</button> : null}
          {onOpenAttention ? <button className="btn" type="button" onClick={onOpenAttention}>Open Needs attention</button> : null}
        </div>
      </section>
    </section>
  );
...
export function NeedsAttentionScreen({
  ...
  return (
    <section className="operational-surface" aria-labelledby="attention-title">
      <div className="page-head"><div><h1 id="attention-title">Needs attention</h1><p>Ambiguous payment, refund, sale, shift, and sync states are reviewed here instead of hidden behind generic errors.</p></div></div>
      {state === "offline" ? <ConnectivityNotice state="offline" /> : null}
      {state === "degraded" ? <ConnectivityNotice state="degraded" /> : null}
      {state === "error" ? (
      ...
      {state !== "loading" && state !== "error" && items.length === 0 ? (
        <div className="card card-pad"><div className="banner success" role="status"><strong>All clear.</strong><span>No mounted recovery source currently reports an item that needs attention.</span></div></div>
      ) : null}
        ...
        <div className="operational-attention-list">
          {items.map((item) => (
            <article className={`card operational-attention-item severity-${item.severity}`} key={item.id}>
              ...
              <p>{item.summary}</p>
              <div className="operational-actions">
                {item.resolveAllowed && onResolveItem ? <button className="btn primary" type="button" onClick={() => onResolveItem(item.id)}>Resolve existing operation</button> : null}
                {item.retryAllowed && onRetryItem ? <button className="btn" type="button" onClick={() => onRetryItem(item.id)}>Retry safely</button> : null}
                {!item.resolveAllowed && !item.retryAllowed ? <span className="muted">No safe automatic action is available. Follow the mounted recovery policy.</span> : null}
              </div>
            </article>
          ))}
        </div>
    ...
    <div className="operational-dialog-backdrop" role="presentation">
      ...
      >
        ...
        <div className="operational-dialog-body stack">
          ...
          <dl className="operational-detail-list"><div><dt>Current build</dt><dd>{currentBuild}</dd></div>{nextBuild ? <div><dt>Ready build</dt><dd>{nextBuild}</dd></div> : null}<div><dt>Critical local data</dt><dd>Preserve</dd></div><div><dt>Catalog projection</dt><dd>Rebuildable</dd></div></dl>
        </div>
...
export function LocalDataMigrationPanel({ state, message, onRetry }: LocalDataMigrationPanelProps) {
  return (
    <section className="card card-pad operational-panel" aria-labelledby="migration-title" aria-live="polite">
      <h2 id="migration-title">Local data upgrade</h2>
      {state === "idle" ? <p>No local data upgrade is currently running.</p> : null}
      {state === "running" ? <div className="operational-state"><div className="operational-spinner" aria-hidden="true" /><div><strong>Preparing local data…</strong><p>Your cart and critical operation references must be preserved.</p></div></div> : null}
      ...
      {state === "failed" ? <div className="banner danger" role="alert"><strong>Local data upgrade needs recovery.</strong><span>{message ?? "Do not clear IndexedDB or critical local records as a normal repair step."}</span></div> : null}
      {state === "complete" ? <div className="banner success" role="status"><strong>Local data updated.</strong><span>Critical cart and operation records were preserved.</span></div> : null}
...
export function FixAppPanel({ criticalOperationActive, onCheckHealth, onRebuildCatalog, onLastResortReset }: FixAppPanelProps) {
  return (
    <section className="card card-pad operational-panel" aria-labelledby="fix-app-title">
      <h2 id="fix-app-title">Fix App</h2>
      <div className="banner info"><strong>Non-destructive recovery first.</strong><span>Critical local business state stays separate from replaceable app caches and rebuildable catalog data.</span></div>
      <ol className="operational-recovery-steps"><li>Check connection and service health.</li><li>Repair replaceable app caches through the WS3 recovery coordinator.</li><li>Rebuild catalog and barcode projection.</li><li>Retry safe local-data repair.</li><li>Use destructive local reset only as a last resort.</li></ol>
      ...
      <div className="operational-actions">
        {onCheckHealth ? <button className="btn" type="button" onClick={onCheckHealth}>Check health</button> : null}
        {onRebuildCatalog ? <button className="btn" type="button" onClick={onRebuildCatalog}>Rebuild catalog</button> : null}
