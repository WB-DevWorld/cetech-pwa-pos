...
final class Cetech_Pos_Bridge_Prepare_Controller {
	...
	public function handle( $request ) {
		...
		try {
			$result = $this->engine->prepare( $body, $key );
		} catch ( \Throwable $e ) {
			$this->log_abort( $e );
			$result = Cetech_Pos_Bridge_Response::wp_error(
				'INTEGRATION_UNAVAILABLE',
				'Prepare runtime aborted before returning a prepared sale.',
				true,
				'resolve',
				503
			);
	...
	private function encode_payload( array $payload ) {
		...
		if ( ! is_string( $json ) || $json === '' ) {
			return Cetech_Pos_Bridge_Response::wp_error(
				'INTEGRATION_UNAVAILABLE',
				'PreparedSale could not be encoded as JSON.',
				true,
				'resolve',
				503
			);
	...
	private function log_abort( $e ) {
		...
		$line  = 'cetech-pos-bridge prepare aborted: ' . get_class( $e ) . ' @ ' . $shown . ':' . $e->getLine();
