...
final class Cetech_Pos_Bridge_Cancel_Controller {
	...
	public function handle( $request ) {
		...
		try {
			$result = $this->engine->cancel( $body, $key );
		} catch ( \Throwable $e ) {
			$this->log_abort( $e );
			$result = Cetech_Pos_Bridge_Response::wp_error(
				'INTEGRATION_UNAVAILABLE',
				'Cancel runtime aborted before returning a sale resolution.',
				true,
				'resolve',
				503
			);
	...
	private function log_abort( $e ) {
		...
		$line  = 'cetech-pos-bridge cancel aborted: ' . get_class( $e ) . ' @ ' . $shown . ':' . $e->getLine();
