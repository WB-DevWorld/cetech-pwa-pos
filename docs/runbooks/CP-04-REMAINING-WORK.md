...
| CP04-W4 | Establish bridge service access for the target runtime. WS2 implements BR-01; an authorized operator provisions a dedicated least-privilege service identity through the approved credential channel. WS3 configures the BFF adapter. | **PASS** on training WordPress 2026-09-13: plugin active, user 22 subscriber, 401/403/200 health, `pricingParityVerified=false`. Secret host-local only. BFF process attach still separate. Evidence `docs/integration/evidence/CP04-W4-BRIDGE-SERVICE-ACCESS.md`. | Before live BR-01/CORE-03 Browser→BFF connectivity acceptance. WordPress side is ready. |
...
Next CP-04 assignment: **CP04-W4 WordPress bridge health is PASS** on training. W1 containment remains in place. R2 still needs a separate Browser→BFF attach using the host-local server-only secret (never `NEXT_PUBLIC_*`) plus durable staff-session composition. Keep issue #4 OPEN. Do not start R3. Evidence: `docs/integration/evidence/CP04-W4-BRIDGE-SERVICE-ACCESS.md`. Rollback: `docs/runbooks/CP04-W4-ROLLBACK.md`.
