...
| Controlled remote write testing | OPEN; training isolation NOT PROVEN; email previously classified UNSAFE | Only the proposed remote operation waits for its necessary safeguards/evidence. No test orders, stock changes, messages or real charges are authorized by this document. |
...
| --- | --- | --- | --- |
| CP04-W1 | Contain outbound side effects. WS3 + training operator inspect and configure mail capture/blocking for Woo and MailPoet, notification integrations, fulfillment and webhook destinations. | Redacted configuration plus a controlled synthetic event demonstrating messages/requests reach only approved test sinks. Record operator, UTC, host and rollback. Existing zero Woo webhooks does not cover other plugins. | Before training events that could notify customers or trigger fulfillment. Continue local implementation and synthetic tests. |
...
| Email, SMS/WhatsApp, fulfillment, webhooks and dataset policy | Training email unsafe; other boundaries not fully proven | WS3 + operator | Applicable remote side effects and production enablement; keep test sinks in nonproduction |
...
Next CP-04 assignment: **CP04-W1 outbound containment assessment/configuration on an explicitly authorized test environment**, coordinated with CP04-W2 and CP04-W3 before any synthetic event. Local CORE-01 and BR-01 implementation continue independently. Follow the [audit runbook](CP-04-STAGING-AUDIT.md) for read-only evidence collection and preserve historical audit conclusions.
