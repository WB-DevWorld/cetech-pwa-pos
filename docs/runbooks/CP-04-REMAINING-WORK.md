...
Next CP-04 assignment: **CP04-W1 outbound containment is BLOCKED** on training (`https://training.cetechbpa.com`) until the operator authorizes MailPoet/admin-mail capture or an approved sink. W2 write-boundary map and W3 health fixtures are recorded; **do not start remote W4** while W1 is blocked or mutation authorization is absent. Evidence: `docs/integration/evidence/CP04-W1-OUTBOUND-CONTAINMENT.md`. Preflight: `docs/runbooks/CP04-W4-BRIDGE-PREFLIGHT.md`. Keep issue #4 OPEN. Follow the [audit runbook](CP-04-STAGING-AUDIT.md) for read-only evidence collection and preserve historical audit conclusions.
