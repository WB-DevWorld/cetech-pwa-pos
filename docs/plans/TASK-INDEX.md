...
CP-04 dependency interpretation: [ADR-011](../decisions/ADR/011.md). Its development baseline is SATISFIED for local CORE-01/BR-01 implementation; CORE-01 is ACCEPTED through R1/`aa08d74f…`. R2 PR #43 is APPROVED / MERGED / VERIFIED on `ab9aa5ae…` (reviewed head `e1a6e005…`; post-merge CI 34765462210 success). BR-01, CORE-02, and CORE-03 are ACCEPTED through that merge. Issue #4 stays OPEN. W1 PASS on training; W4 PASS on training WordPress side. Overall CP-04 is not complete. R3 is explicitly activated; BR-02 (issue #14) is the first executable WS2 task. An open tracker is not a blanket dependency failure. [Remaining actions](../runbooks/CP-04-REMAINING-WORK.md).
