...
Local CORE-01 schema (`supabase/migrations`) owns POS operational tables only: organizations, locations, devices, registers, staff assignments, shifts, cash movements, pending operations, outbox events, and rebuildable integration watermarks. It does not own products, Woo customers, price rules, WoodMart/B2BKing commercial rules, Woo orders, payment-provider settlement, or live Woo inventory. External references use provider-neutral identifiers and snapshots; there are no cross-database foreign keys. Expected cash is nonnegative contract Money; a movement that would make it negative is rejected atomically under a shift row lock (`SELECT ... FOR UPDATE`). Opening float initializes expected cash; the generated `opening_float` ledger row is already represented and is not added twice. Authenticated cashier cash kinds are `pay_in` / `pay_out` / `cash_pickup` / `correction` and require a reason; internal kinds `opening_float` / `cash_sale` / `cash_refund` are trusted-server only. Incompatible cash currency is rejected. Command idempotency is claimed on `pos_pending_operations` as `organization_id + operation + idempotency_key`; the cash row does not implement a second competing key. Authoritative shift close / immutable Z orchestration is CORE-07; CORE-01 stores expected/count/variance, one-active-shift, closed-shift immutability, and non-deletable shift history only. The transactional outbox and integration watermarks are trusted-server surfaces; authenticated clients cannot select or insert them. Register/shift/cash SELECT remains location-scoped in CORE-01; finer cashier-versus-manager register operational-read distinction is deferred to CORE-02 because JWT capability/manager semantics are not frozen enough to guess a manager policy. Same-shift cash writers serialize via row lock; a true two-session concurrency harness is recorded for CORE-05/QA-01 because pgTAP cannot safely orchestrate parallel sessions.
