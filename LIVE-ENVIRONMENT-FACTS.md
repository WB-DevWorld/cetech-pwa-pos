...
| WordPress bridge service identity/capabilities | Route present; unauthenticated health 401 `AUTH_REQUIRED`. Authenticated identity, scope, and write authorization remain OPEN. | VERIFIED 2026-09-15 unauthenticated `GET https://training.cetechbpa.com/wp-json/cetech-pos/v1/health` → HTTP 401 JSON `ok:false`, `error.code=AUTH_REQUIRED`. This supersedes the 2026-09-12 HEAD 404 “plugin absent” observation. 401 is not write authorization and does not close CP04-W4. Application Passwords advertised on staging REST (2026-09-12). |
