...
| Staging isolation vs production writes | NOT PROVEN for production-delta; training **email contained** for CP04-W1 | Production fingerprint still unavailable. Woo webhooks 0 VERIFIED 2026-09-13T06:05:13Z. MailPoet 5.37.0 **inactive** after authorized W1 apply. `admin_email` / stock-notify / new-order recipient domains **`training.invalid`**. MU intercept captures `wp_mail`. Synthetic correlation captured; mail queue empty. Isolation vs production data stores still NOT PROVEN. See `docs/integration/evidence/CP04-W1-CONTAINMENT-APPLY.md`. |
