...
## Ownership-preserving continuation (ADR-014)

[Canonical rule](docs/plans/LONG-RUNNING-WORK.md#ownership-preserving-milestone-execution). Task ownership and human identity survive batching. A milestone lease may list the union of paths for declared imports, but its editor gains implementation rights only to that editor's own scoped tasks. Source/import metadata alone does not prove the human implementer. WS3 integrates WS1/WS2 commits without taking their behavior changes; review findings return to the relevant owner. Unavailability is not reassignment. Explicit senior reassignment must be recorded in CURRENT-WORK before an exception is used.

The 2026-09-13 WF-OWN-01 correction grants WS3 only a governance/docs lease: agent/Cursor instructions, ADR/policy/register, ownership/ledger, templates/role prompts, and workstream queue/planning guidance. No WS1/WS2 product code, R4 implementation, contracts, CI configuration, dependencies, migrations or runtime effects are authorized. The R1 adoption lease above is historical, not ongoing implementation authority.
